...
from wxmm.fairvalue.reconstruction_bound import ReconstructionBound
...
def test_market_mid_refuses_without_bound(tmp_path: Path) -> None:
    view = _view()
    ctx = BaselineContext(
        climate_day="2026-07-04",
        doy=186,
...
def test_market_mid_with_bound(tmp_path: Path) -> None:
    view = _view()
    ctx = BaselineContext(
        climate_day="2026-07-04",
        doy=186,
...
def test_load_bound_from_committed_artifact() -> None:
    path = Path("analysis/out/emission_convention_sweep.json")
    if not path.is_file():
        pytest.skip("committed sweep artifact not present")
    from wxmm.fairvalue.reconstruction_bound import load_reconstruction_bound

    bound = load_reconstruction_bound(path)
    assert bound.branch in {"full_corpus", "forward_logger_only"}
