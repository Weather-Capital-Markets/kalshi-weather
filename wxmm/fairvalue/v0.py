    ...
    for climate in train_days:
        ...
        for hours in hours_to_close:
            ...
            ladder = trade_ladder_or_none(
                trades, tickers, as_of=as_of, mapping=mapping
            )
            if ladder is None:
                continue
...
def run_c1_m1_v0(
    ...
    *,
    ...
    origins = list(
        expanding_origins(climate_days, min_train_days=min_train, hours_to_close=hours)
    )
    ...
    for origin in origins:
        ...
        if won_ticker is None or len(tickers) < 2:
            continue
        ladder = trade_ladder_or_none(
            trades, tickers, as_of=origin.as_of, mapping=mapping
        )
        if ladder is None:
            continue
        ...
        predictions.append(
            V0Prediction(
                ...
                rps_climatology=(
                    ranked_probability_score(clima, won_ticker) if clima else None
                ),
